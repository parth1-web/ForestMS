...
| **Stack** | ASP.NET Core 3.1 MVC (Areas) · EF Core + Npgsql (PostgreSQL) · Autofac · Rotativa (view rendering) + headless Chromium PDF engine |
...
### Reporting (PDF)
- All PDF downloads (journal, ledger-group, account-statement, day-book, balance-sheet, trial-balance, profit-loss, trading, receipt, payment, wood-stock, sales, piling-wise) render server-side through `LE.Web/Helpers/ChromiumPdfResult.cs` — a drop-in `ViewAsPdf` subclass that prints the same Razor HTML with headless Edge/Chrome/Brave
- Full Nepali (Devanagari) Unicode support; wkhtmltopdf is no longer used (it cannot shape Devanagari)
- Shared `Views/Shared/_PdfNepaliFont.cshtml` partial forces Nirmala UI on PDF views
- Requirement: a Chromium browser (Edge, Chrome, or Brave) installed on the host — auto-detected, no extra packages
...
- PDF engine migrated from wkhtmltopdf to headless Chromium (`ChromiumPdfResult`): Nepali text now renders correctly in all 13 PDF reports; requires Edge/Chrome/Brave on the host
- Journal entry screen: visible Add Row button (previously Enter-key only)
- Wood bill report: stock-type filter is now optional (`/report`, `/report-print` work with or without a type; blank = all types); print button carries the current type
- Counter report print button repointed to `/billing/counter/report-print`
- Razor runtime compilation enabled in Development — `.cshtml` edits apply on browser refresh without a rebuild
...
- PostgreSQL 12+
- A Chromium browser on the host (Microsoft Edge, Google Chrome, or Brave) — auto-detected for PDF printing (wkhtmltopdf is no longer used)
...
├── Helpers/               → ChromiumPdfResult (PDF engine), LoginAttemptTracker, ModulePermissionFilter, ExceptionMessageHelper, etc.
